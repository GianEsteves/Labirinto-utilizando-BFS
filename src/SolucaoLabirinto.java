...
public class SolucaoLabirinto {
    ...
    public static void BFS(int[][] labirinto, Ponto inicio, Ponto fim) {
        int rows = labirinto.length;
        int cols = labirinto[0].length;
        ...
        Queue<Ponto> queue = new LinkedList<>();
        queue.add(inicio);
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (isSafe(labirinto, newRow, newCol, visited)) {
                    queue.add(new Ponto(newRow, newCol));
